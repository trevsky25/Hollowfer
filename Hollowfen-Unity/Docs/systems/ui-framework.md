...
Key scripts: `Assets/_Hollowfen/Scripts/UI/` — UIManager, UIScreen, MainMenuScreen, SettingsScreen, SaveSlotScreen, ConfirmModal, LoadingScreen, FocusHighlight, UICanvasUtil, HollowfenPalette; scene-local overlays NarrationOverlay (sort 70) + IntroGuide (sort 65, batch-30 first-steps journal page, restyled batch-31: offset-LEFT page with the world visible right, `UICanvasUtil.PaperGrain` texture + inset gold frame + ledger double-rule + keycap chips; once per save via `intro_guide_seen`, triggered by StoryBeats after the intro).
