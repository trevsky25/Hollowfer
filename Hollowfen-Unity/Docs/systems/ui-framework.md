...
Key scripts: `Assets/_Hollowfen/Scripts/UI/` — UIManager, UIScreen, MainMenuScreen, SettingsScreen, SaveSlotScreen, ConfirmModal, LoadingScreen, FocusHighlight, UICanvasUtil, HollowfenPalette; scene-local overlays NarrationOverlay (sort 70; two modes — `Show(...)` = captions on full black for the act-break journal narration; `ShowCinematic(captions, clips, hero, onDone)` = batch-36 opening, captions in the lower third over a Ken-Burned hero image with letterbox bars + a procedural bottom scrim. The homecoming intro (`StoryBeats`) uses it with `homecoming.png` + the bible's fuller Scene-1 passage restored) + IntroGuide (sort 65, batch-30 first-steps journal page, restyled batch-31: offset-LEFT page with the world visible right, `UICanvasUtil.PaperGrain` texture + inset gold frame + ledger double-rule + keycap chips; once per save via `intro_guide_seen`, triggered by StoryBeats after the intro).
...
| `MenuCinematics` (batch-35) | `_Hollowfen/Scripts/UI/MenuCinematics.cs` (on the MainMenuScreen Canvas) | **Cinematic Pass #1 — "living painting."** Runtime-procedural, UI-native (the canvas is Screen-Space-Overlay, so no post-FX Volume applies): drifting golden **spore motes** (twinkle + sway + wrap), soft **mist**, a full-screen **vignette + warm grade** (faked post-processing that composites in overlay), **Ken Burns** ping-pong scale/drift on `BG_WrenImage`, and a staggered **ink-bleed reveal** of the title block on open. Finds elements by name (no serialized wiring), generates its sprites (`Texture2D` radial) — no new art, works in builds. Restraint is the rule: a title glow was cut for reading cheap on the strong hero art. |
...
| `LoadingScreen` | `_Hollowfen/Scripts/UI/LoadingScreen.cs` | Two modes. **Continue/Load**: plain "Traveling to Hollowfen…" rolling dots. **New game (batch-38 seamless opening)**: a cinematic **welcome** card — `homecoming.png` at the narration's Ken-Burns A-state + letterbox + a "CHAPTER ONE / Homecoming" title that pops up + a discreet loading line. `UIManager.LoadSceneAndOpen(scene, next, cinematicHandoff:true)` (from `SaveSlotScreen` new-game only) loads Scene_Hollowfen behind it, **holds until the in-scene `NarrationOverlay` is showing** (which now starts opaque at the same A-state), then `FadeOutAndClose` cross-fades the card out → image→image seamless into the intro → game. Needs a `CanvasGroup` on its Canvas child (wired). |
