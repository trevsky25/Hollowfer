...
Key scripts: `Assets/_Hollowfen/Scripts/UI/` — UIManager, UIScreen, MainMenuScreen, SettingsScreen, SaveSlotScreen, ConfirmModal, LoadingScreen, FocusHighlight, UICanvasUtil, HollowfenPalette; scene-local overlays NarrationOverlay (sort 70) + IntroGuide (sort 65, batch-30 first-steps journal page — once per save via `intro_guide_seen`, triggered by StoryBeats after the intro, parchment card + live quest block + 'Set out →').
