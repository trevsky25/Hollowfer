...
Data: `Data/NPCs/NPC_<Name>.asset` — Bram, Marra, Almy, Joren, Voss. Ids lowercase (`voss`), matching relationship ids in dialogue/quest score deltas.
Entry conditions (ANDed, unset = skip): `activeQuest` (IsActive) · `requiresQuestCompleted` · `requiresFlagId` (GameScores) · `requiresCoinsCopper` (≥) · `requiresBasketNonEmpty` (InventoryRuntime).
...
Status: verified against code 2026-07-11. Remaining cast to build: Edda, Theo, Hollin, Father Calden, Lord Aldric.
