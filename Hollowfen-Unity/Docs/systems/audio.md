...
Batch-39: the **full 6-beat homecoming intro is voiced** (`HomecomingIntro/00..05_Narrator`, Narrator=`af_heart`@0.86, index-matched to `StoryBeats._introVoiceClips`) and **Bram revoiced older** (`bm_lewis`@0.86, was `bm_george`). Trevor directed opening VO (QUESTIONS Q10). Voices live in `generate_vo.py` VOICES; re-run the scratchpad Kokoro venv to regenerate. WAVs are git-tracked. **Batch-62: the hidden-journal reveal is voiced** — 7 Narrator (`af_heart`) clips at `VO/HiddenJournal/00..06_Narrator.wav`, index-matched to the captions split from `act1.hidden_journal.tobin_note`, wired to the journal's `QuestInteractable._narrationVoiceClips` and played through `NarrationOverlay.ShowCinematic`. Whole passage is Wren's narration voice (it's framed as her reading her father's journal); a distinct Tobin voice for his farewell lines is a parked casting call (QUESTIONS Q10). Regenerate with `generate_vo.py --journal-only`.
