...
Data: `Data/Locations/LocationData_*.asset` (8 POIs; only FathersMill `_discoveredByDefault`); discovery key = `_id`, names/descriptions via localization keys (`loc.<id>.name/.desc`).
