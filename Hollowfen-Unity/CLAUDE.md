...
| `MapCamera` | `_Hollowfen/Scripts/Map/MapCamera.cs` | Wider ortho camera (size 150 = 300m vertical frame) following Player at 80m. Always active. **Creates its own runtime RenderTexture in `Awake()` at the configured `_renderTextureSize` (default `2048×1024` = 2:1 landscape) and assigns it to `Camera.targetTexture`** — overrides any project RT asset wired in the inspector (the asset becomes orphaned but harmless). Public `RenderTexture` getter so MapScreen can pull it without traversing the camera. Subscribes to `RenderPipelineManager.beginCameraRendering` to disable scene fog only while it renders — otherwise URP fog washes the view to skybox-blue at altitude. |
| `MapScreen` | `_Hollowfen/Scripts/Map/MapScreen.cs` | Lazy programmatic builder following the parchment journal aesthetic of Inspect/Inventory. Toggles the full-screen canvas, hides the mini-map canvas, freezes `Time.timeScale`. Not a `UIScreen` subclass — runs independently of UIManager since UIManager is DDOL'd from MainMenu. Layout: `1620×920` panel with vignette + outer/inner gold double-rule frame; top header band (110px) with `FIELD JOURNAL · CARTOGRAPHY` eyebrow + Georgia serif "Hollowfen" + gold rule; map zone `1480×740` (2:1, matches RT aspect — no stretch); 8 compass markers in dark pills around the perimeter (`N/S/E/W` in `GoldGlow` 16px bold, intercardinals in `Gold` 13px); hint band at bottom. `BuildIfNeeded()` runs on first Open — tears down any pre-existing children of the canvas (legacy hand-authored UI) and rebuilds; pulls the texture from `MapCamera.RenderTexture` for the new RawImage. Heading triangle (`UITriangle` + `PlayerHeadingArrow`) is re-created in code, not preserved across rebuilds. |
...
- `_MapCanvas` — deactivated by default (sortingOrder 50). Children are now built programmatically by `MapScreen.BuildIfNeeded()` on first open; the legacy hand-authored hierarchy (Background + MapPanel + Frame + 4 Cardinal_* + floating Title + Hint) was torn down. Final shape: parchment panel + vignette + double-rule frame + header band + 1480×740 map zone + 8 compass pills + hint band.
...
| `InventoryRuntime` | `_Hollowfen/Scripts/Foraging/InventoryRuntime.cs` | Static, in-memory dictionary of mushroom-id → count. `Add(data, n)` and `Remove(data, n)` mutate counts and fire `OnChanged(id, newCount)`. Lazy-hydrates from `SaveSlotMeta.Inventory` on first read (autosave slot 0). `Persist()` calls `SaveManager.AutoSaveInventory(snapshot)` after every change — every harvest = one tiny JSON write. SO refs cached in a parallel `_byId` map so UIs can render photos without re-resolving on every frame. |
...
| `InventoryScreen` | `_Hollowfen/Scripts/Foraging/InventoryScreen.cs` | Scene-local screen at `sortingOrder = 55` (below Inspect's 60). Same parchment journal panel + double-rule frame + vignette as InspectScreen. Title "Provisions" with gold underline. Empty state copy when `DistinctCount == 0`. Otherwise: 4-col `GridLayoutGroup` of mushroom cards (170×220) wrapped in a `RectMask2D` ScrollRect (752×660) on the left; selected card recolors its 4-edge border to gold; click / D-pad selection drives `MushroomPreviewer.Show(cell.Data, false)`. Right side: 600×600 preview frame with hint pill at the top, "MODEL COMING SOON" overlay shown when `data.WorldPrefab == null`, and selected-mushroom title + Latin + count badge underneath. Bottom: single Close button (auto-detect `○`/`B`/`A`/`Esc` glyph). Subscribes to `InventoryRuntime.OnChanged` so harvests while open refresh the grid live. **Inspect Mode toggle**: R1 / Tab flips between Browse (default — gamepad navigates cards via UI/Navigate, camera locked) and Inspect (camera unlocked: left-stick pans, right-stick rotates, LT/RT zooms; EventSystem selection is cleared so left-stick is freed). Preview frame border brightens to full gold + hint text changes to communicate the active mode. Cancel handler routes through Inspect Mode first (B exits inspect; second B closes the screen). |
| `MushroomFocusCamera` | `_Hollowfen/Scripts/Foraging/MushroomFocusCamera.cs` | Cinematic "discovery shot" via Cinemachine 3. Single `CinemachineCamera` + `CinemachineFollow` + `CinemachineRotationComposer` on `_MushroomFocusCamera`. Default priority 5 (below `PlayerFollowCamera`'s 10); when `PlayerInteractor.OnFocusChanged` fires with a non-null target, raises priority to 20 → CinemachineBrain blends to the framed shot at the configured `_blendSeconds = 0.55`. Lens FOV `38°`, follow offset `(0.7, 0.7, -1.6)` from the mushroom for an over-the-shoulder hero framing. Manual orbit: right-stick (gamepad) + RMB-drag (mouse) yaw + clamped pitch (`-10°..60°`). UI/Cancel binding calls `PlayerInteractor.DismissCurrent()` to back out of the zoom. Also exposes `IsHarvestCinematicActive` so `MushroomNode.HarvestCinematic` can take ownership of FollowOffset during the pickup animation. |
...
- **Inventory persistence**: `MushroomNode.Harvest` calls `InventoryRuntime.Add(_data, 1)` after marking discovery. `InventoryRuntime` writes the snapshot to `saves/slot0.json` (autosave slot) immediately. On every gameplay launch, `Continue` from MainMenu → Scene_Hollowfen reload → `InventoryRuntime.EnsureHydrated` reads back the snapshot. The slot file contains the existing `SaveSlotMeta` plus a new `InventorySnapshot { Ids[], Counts[] }`. To reset state during dev, delete `Application.persistentDataPath/saves/slot0.json` and call `PlayerPrefs.DeleteAll()`.
- **`SaveManager.AutoSaveInventory(snap)`** is the targeted write path — reads the existing slot meta, swaps `Inventory`, refreshes timestamp, writes back. Doesn't clobber `CurrentQuest` / `CurrentAct` / `TotalPlayTimeSeconds` (those will get their own gameplay-side autosave hooks later).
- **`Player/OpenJournal` was renamed to `Player/OpenInventory`** since the inventory IS the journal-as-shipped-today. Bindings: keyboard `I`, gamepad **Square / X** (`buttonWest`). Old OpenJournal references in code are gone. If a real journal feature ever ships, it can take a different action slot.
- **Three species modeled**: Field Mushroom (id `fieldMushroom`, ground), Fly Agaric (id `flyAgaric`, ground), Oyster (id `oyster`, mounted to tree trunks). All share the same URP-Lit-from-Meshy recipe + prefab structure. Oyster prefabs are placed via `Quaternion.FromToRotation(Vector3.up, outwardNormal)` so the mesh's local +Y substrate axis aligns with the trunk's surface normal — works because Meshy's Oyster export has its growth axis on local Y. The `Mushroom_17_Oyster` SO was authored from scratch (the original web import only included 16 species) and is the 17th entry of `MushroomFieldGuideDatabase`.
- **Mushroom centering on bounds.center**: each Meshy export pivots at the mesh BASE, not the visual center. `MushroomPreviewer.Show` computes renderer-bounds for the spawned child and shifts its world position so `bounds.center == mount.position`. Without this the camera (which aims at the mount) frames the base of the mushroom and the visible model sits in the upper part of the preview rect. Works across all three species despite different bounds.
- **Cinematic harvest sequence** (`MushroomNode.BeginHarvest`): triggered by Forage button on InspectScreen instead of synchronous Harvest. Closes screen via `HideForCinematic` (which deliberately doesn't restore time/Suspended/cursor — coroutine owns them). Coroutine runs at `Time.unscaledDeltaTime`: phase 1 (0.30s) lerps the focus cam's FollowOffset to 55% of current (dolly in) under `Time.timeScale = 0.5` slow-mo; phase 2 (0.80s) lifts the mushroom 25cm + spins 360°/s on Y + scales to 5%; commit happens at the end (logs, fires events, marks discovery, adds to inventory, fires achievement) BEFORE `gameObject.SetActive(false)` so coroutine cleanup runs on a still-active host.
- **`PlayerInteractor.SetPlayerInputEnabled(bool)`** toggles Wren's StarterAssets `PlayerInput` component. Used by InspectScreen.Open/Hide, InventoryScreen.Open/Close, and the harvest cinematic. Without it, Cross/Space confirms Forage AND queues a `Player/Jump` on the same physical button — buffered until `Time.timeScale` resumes, then Wren visibly jumps in place after every harvest. Disable on Open, re-enable on Close (or at the end of the harvest cinematic).
- **`PlayerInteractor.DismissCurrent()`** is the "back out of cinematic zoom" hook for `MushroomFocusCamera.OnCancel`. Saves the current target into `_suppressed` and clears focus. The next `Update()` skips that target while it's still in OverlapSphere range; once Wren walks out and the target stops appearing in hits, suppression clears so re-entering the trigger works normally.
- **Inventory Inspect Mode toggle**: solves the conflict where gamepad left-stick is bound BOTH to UI/Navigate (card grid) AND to camera pan. Default mode is Browse — left-stick navigates cards, gamepad camera input is gated off. Press R1 / Tab → Inspect Mode: `EventSystem.SetSelectedGameObject(null)` (so UI/Navigate has nothing to navigate from, freeing left-stick), camera input gate opens, preview frame border brightens to full gold, hint text changes. R1 again or Cancel exits back to Browse with the previous card selection restored. Mouse always has full camera control regardless of mode (left-drag rotate, middle-drag pan, scroll zoom inside the preview rect — these don't conflict with card clicks because cards are outside the preview rect).
- **Camera pan in previewer**: `MushroomPreviewer.ApplyPanDelta(Vector2 cameraXY)` shifts both the camera position AND the look target by the same world-space vector along `cam.transform.right * x + cam.transform.up * y`. Result is true lateral pan (gaze stays parallel) rather than orbit. Pan composes cleanly with rotate (which spins the mount around its origin) and zoom (which only changes ortho size). `ResetView()` zeroes pan along with yaw/pitch/ortho — called on every `Show()` so each new mushroom starts framed cleanly.
