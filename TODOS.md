...
- **Hardening pass (from the 2026-07-11 doc-backfill audit)** — (a) ordinary quest achievements remain story-card-dependent; add per-quest hooks. (b) ~~non-atomic save writes~~ **DONE with ending engine** — temp/replace/backup + corrupt-primary recovery. (c) ~~`TimeManager` static events lack reset~~ **DONE with game-loop foundation**. (d) `CurrentQuestId` now ships alongside the legacy display cache; migrate SaveSlotScreen to resolve the ID at render time, then retire `CurrentQuest`. (e) Dead-code sweep: `MapScreen.BuildFrame`, `DialogueScreen.BuildFrame`, `QuestBootstrap._startDelaySeconds`, `QuestData._order`. One focused batch.
