...
**2026-07-21 update (Batch 122):** The first production task on that command layer traced all 36 gameplay negative-scale collider findings to zero-bounds, invisible `BrokenWood2` proxies nested in mirrored rampart prefabs. Hollowfen now disables only those components through scene-instance overrides, preserves the imported sources and 376 functional colliders across the 11 affected wall roots, and keeps the disabled findings visible in a separate audit bucket. Native preflight plus Coplay integrity/smoke remain green.

**2026-07-21 update (Batch 123):** The first shipping-debug cleanup removes the empty duplicate `Save 1`/`Steam 1` asset folders, the unattached coroutine-era `UITestDriver`, and the inactive gameplay `LocationDebugHUD` that still compiled legacy Input/IMGUI/discovery mutation into Player assemblies. Player compilation now contains zero retired debug sources and gotcha lint is clean with zero waivers; the production gate and Coplay smoke remain green.

**2026-07-21 update (Batch 124):** Save-slot quest presentation is now authoritative from `CurrentQuestId`: current writers strip the obsolete localized cache, canonical IDs resolve through data-integrity-enforced `quest.<id>.name` keys, terminal states are explicit, unknown current IDs cannot revive stale text, and id-less schema-zero journals retain their historical label until a full save upgrades them. Native save/UI verification and Coplay cross-check remain green.
...
11c. ~~**Gameplay collider and scene health**~~ **DONE 2026-07-21, tag `batch-122`** — classified all 36 native world-audit findings as enabled but zero-bounds `BrokenWood2` proxy colliders under 11 mirrored rampart roots; disabled only those components with scene-instance overrides, preserved vendor sources and 376 functional wall colliders, and split active hazards from disabled findings in the Editor-only audit. Menu-to-gameplay boot, preflight, integrity, and smoke are clean.
...
13. **Ending engine — IMPLEMENTED + VERIFIED 2026-07-15; awaiting commit/tag before queue-DONE.** Four data-authored score/flag/relationship gates; disabled decision branches with reasons; exclusive `game_complete`; one chosen card; final dialogues; painted epilogues; runtime credits with Return/Remain; ending achievements; atomic final save. Four-branch verifier + 1280×800 presentation proof green.
13b. **Game-loop foundation — IMPLEMENTED + VERIFIED 2026-07-16; awaiting commit/tag before queue-DONE.** Rest crosses real sundown/dawn events; wild nodes save cut days and respawn by species; 21 species own tier/gate/respawn/buyer/cultivation data; repeat Marra/Theo sales preserve refused stock; grow beds share a four-recipe picker. Focused verifier green with zero console errors and exact save restoration.
13c. **Village requests — IMPLEMENTED + VERIFIED 2026-07-16; awaiting commit/tag before queue-DONE.** Three deterministic daily orders each for Marra/Edda/Theo, illustrated controller request card + tracker, atomic deliveries, first-only relationship/knowledge, save round-trip, and the canonical Goldfoot/Lacewig/Field Cap/Brightspore festival gathering. Remaining festival work is physical square dressing only.
...
- ~~**Actual day/night lighting cycle**~~ **DONE batch-82** — continuous sun/moon, trilight ambient, procedural-sky exposure/tint, fog, reflections, time-of-day color grade, and six warm shadowless village practicals. Focused verifier and fixed-view day/dusk/night visual QA green. Visible lantern/window fixtures can be refined during world dressing without rewriting the cycle.
- **Hardening pass (from the 2026-07-11 doc-backfill audit)** — (a) ordinary quest achievements remain story-card-dependent; add per-quest hooks. (b) ~~non-atomic save writes~~ **DONE with ending engine** — temp/replace/backup + corrupt-primary recovery. (c) ~~`TimeManager` static events lack reset~~ **DONE with game-loop foundation**. (d) ~~CurrentQuest-as-ID~~ **DONE batch-124** — save-slot rendering and current writes are ID-authoritative; `CurrentQuest` remains only for id-less schema-zero compatibility. (e) Dead-code sweep: `MapScreen.BuildFrame`, `DialogueScreen.BuildFrame`, `QuestBootstrap._startDelaySeconds`, `QuestData._order`. One focused batch.
...
- ~~**Mushroom mesh decimation** — Field Mushroom is 414k verts; decimate all Meshy exports before EA.~~ **DONE batch-68** — all 16 delivered source models now generate 12k–16k-triangle world and 60k–75k-triangle journal derivatives; source art remains untouched.
...
- ~~**Region-enter toasts**~~ **DONE batch-84** — shared `RegionCatalog` copy, unscaled non-blocking arrival title, and real RegionChanged routing to map/audio/UI.
- **Input asset consolidation** — merge StarterAssets map into project InputActions when a concrete need arises.
- ~~**Mushroom tier/economy/cultivation engine**~~ **FOUNDATION DONE batch-80** — all 21 species have tier gates, respawn cadence, Marra/Theo values, and cultivation metadata; four recipes are live. Remaining world-dressing work: place the currently unrepresented species in believable regions and tune density after a traversal/performance pass. T5 stays bible-reserved.
- ~~**Wendlight world prefab remodel** — replace the mystical stand-in with a real Liberty Cap.~~ **DONE batch-68** — Wendlight now has a species-specific world prefab using the delivered *Psilocybe semilanceata* model.
- ~~**Theo inn staging pass**~~ **DONE batch-83** — Theo now derives wagon/Pintle placement from saved story + clock state: daytime wagon trade, post-`eddaApprentice` evenings at the Pintle, and an all-day active-quest override for `theoCapitalOffer`; the quest waypoint now targets the Pintle. This became the reusable `NPCSchedule` engine rather than another parallel flag-placement special case.
- **Festival world-dressing pass** — ~~gather gate/system~~ **DONE batch-81** with the four canonical dishes and an obtainable Lacewig source. Remaining: lanterns, serving tables, crowd silhouettes, festival dressing driven by `festival_hosted`, and a proper square trigger. A real cooking/timer minigame is optional future scope, not required by the working gathering.
...
- **Audio pass** — **regional foundation DONE batch-84**: six trigger volumes, four procedural 48 kHz day/night profiles, equal-power ambience crossfades, adaptive inherited score states, and Ambience setting routing. Remaining: authored regional compositions if desired, gameplay SFX on existing events, dedicated Voice mixer/slider, and full-cast VO pending Q10.
- **Build cleanup sweep** — **shipping-debug subtask DONE batch-123:** removed empty `Save 1`/`Steam 1`, retired the unattached `UITestDriver`, removed inactive `LocationDebugHUD` legacy Input/IMGUI/discovery mutation, and eliminated the final lint waiver. **Negative-scale collider subtask DONE batch-122:** 36 zero-bounds `BrokenWood2` proxies disabled through scene overrides while 376 functional wall colliders remain. Remaining: TMP migration in ConfirmModal/SaveSlot/Loading screens (+ MainMenu — SettingsScreen DONE in batch-28, use it as the template), 31 canon locations pass, content-vs-bible sweep, and a focused Unity 6.4 Editor-tool deprecation pass (currently nine Hollowfen warnings: `GetInstanceID`, `FindFirstObjectByType`, and sorted `FindObjectsByType`). Re-diagnose any future vendor Terrain/MeshCollider warning before changing geometry.
