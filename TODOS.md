...
- **Build cleanup sweep** — leftover `Save 1`/`Steam 1` folders, retire `UITestDriver`, legacy Input usage in `LocationDebugHUD`, TMP migration in ConfirmModal/SaveSlot/Loading screens (+ MainMenu — SettingsScreen DONE in batch-28, use it as the template), 31 canon locations pass, content-vs-bible sweep. **Negative-scale collider subtask DONE in batch-122:** the current gameplay audit found 36 (not the historical 44) enabled but zero-bounds `BrokenWood2` proxies under mirrored rampart art; scene-instance overrides disable them without touching vendor sources, while 376 functional colliders remain across all 11 affected roots. Re-diagnose any future vendor Terrain/MeshCollider or other startup warning before changing geometry.
